...
var block = {
  ...
  text: /^[^\n]+/,
  wtlink: /\[([^@#!]{1}[^\[\]\|]{0,})(\|){1}([^\[\]]{0,}){0,1}\]|([\s]){0,1}(https{0,1}:\/\/[^\s]{0,})|\[(https{0,1}:\/\/[^\]]{0,})\]/, // add by wh
...
Lexer.prototype.token = function(src, top, bq) {
  ...
  while (src) {
    ...
    if(this.options.wtlink && (cap = this.rules.wtlink.exec(src))) {
      ...
      this.tokens.push({
        type: 'wtlink',
        href: cap[1] || cap[5] || cap[6],
        link: cap[3] || cap[1] || cap[5] || cap[6],
        space: cap[4]
      });
...
// add by wh
Renderer.prototype.wtlink = function(href, link, space) {
  var out = '';
  if(space) {
    out += ' ';
  }
...
// add by wh
Renderer.prototype.wthexcolor = function(color) {
  var out = color + '<div class="" style="background:' + color + '"></div>';
  return out;
};

// add by wh
Renderer.prototype.wtat = function(href, link) {
  var out = '<a href="'+ href + '" class="" >@' + link + '</a>';
  return out;
};

// add by wh
Renderer.prototype.wthash = function(href, link) {
  var out = '<a href="'+ href + '" class="" >' + link + '</a>';
  return out;
};

// add by wh
Renderer.prototype.wtexclamation = function(key, text) {
  if(key === 'channel' || key === 'group') {
    text = '@' + text;
  }
  var out = '<span class="mention ' + key +  '">' + text + '</span>';
  return out;
};

// add by wh
Renderer.prototype.wtemoji = function(text) {
  return text;
};
...
Parser.prototype.tok = function() {
  switch (this.token.type) {
    ...
    case 'wtlink': {// add by wh
      return this.renderer.wtlink(this.token.href, this.token.link, this.token.space);
    }
    case 'wthexcolor': {// add by wh
      return this.renderer.wthexcolor(this.token.color);
    }
    case 'wtat': {// add by wh
      return this.renderer.wtat(this.token.href, this.token.link);
    }
    case 'wthash': {// add by wh
      return this.renderer.wthash(this.token.href, this.token.link);
    }
    case 'wtexclamation': {// add by wh
      return this.renderer.wtexclamation(this.token.type, this.token.text);
    }
    case 'wtemoji': {// add by wh
      return this.renderer.wtemoji(this.token.text);
    }
  }
};
