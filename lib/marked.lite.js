...
//new YB em: /^\b_((?:[^_]|__)+?)_\b|^\*((?:\*\*|[\s\S])+?)\*(?!\*)/,
  ...
  wtat: /^\[@(\w*)+\|([^\]]+)\]/, // add by wh
  wthash: /^\[#(\w*)+\|([^\]]+)\]/, // add by wh
  wtentity: /^\[#(task|event|file)(-)(\w*)+\|([^\]]+)\]/, // add by macj
...
Renderer.prototype.code = function(code, lang, escaped,origin) {
  if(this.options.isCodeDefault){
    ...
    var dest = self.liteMarked(code, {renderer: self.renderer});
    return dest;
  }
...
Renderer.prototype.wtentity = function(_origin,_type, _id, _name, prefix){
  ...
  out += '<a token="'+_origin+'" class="hand '+this.options.wtentityRender.className+' lcfont lc-link-'+_type+' marked-entity">' + _name + '</a>';
  return out;
};
// add by wh
